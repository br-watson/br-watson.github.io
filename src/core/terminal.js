export function createTerminal({
	...
	const s = (className, text) => ({ type: "span", className, text });
	const l = (href, text) => ({ type: "link", href, text });
	function boot() {
		printBanner();
		printLine(
	...
	function printLine(content, cls = "ok") {
		const segments = Array.isArray(content) ? content : [t(String(content))];
		renderer.printLine(segments, cls);
	}
	function printPre(text, cls = "ok") {
		renderer.printPre(String(text), cls);
	}
	...
	function listHome() {
		const children = Object.keys(getHomeDir().children);
		return children.sort();
	}
	function resolveHomeItem(name) {
		return getHomeDir().children[name];
	}
	...
	async function run(inputLine) {
		...
		if (!line) return;
		printCmd(line);
		const parsed = tokenizeWithMeta(line);
	...
	function tokenizeWithMeta(line) {
		const tokens = [];
		let cur = "";
		let quote = null;
		let escaping = false;
	...
	function autocomplete(current) {
		const parsed = tokenizeWithMeta(current);
